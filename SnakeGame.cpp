...
void random_number(int arr[], int position);                   //create random numbers between [1-6]. This function represents the (zari). The number represents how many steps the player will move forward
void checkLader_Snake(int pinaka[], int position);            // check if the player steps into a block with lader, or a block with snake. It changes his position respectively
...
int main()
{
    ...
    {
        random_number(players, pos);      //create random number from the die (zari), and put it in the respective position
        cout << "The player " << pos << " moved to " << players[pos] << "th STEP" << endl << endl;
        checkLader_Snake(players, pos);
...
void random_number(int arr[], int position)
{
    int zari = 0;

    zari = 1 + (rand() % 6);   //create random numbers from 1 to 6 [1-6], to move the player based on that number
    cout << "The die(zari) is equal to: " << zari << endl; //print what the die(zari) is every time it turns
    arr[position] = arr[position] +  zari;   //add the result to exact player
}

void checkLader_Snake(int pinaka[], int position)
{
